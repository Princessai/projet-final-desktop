...
function Timetable({ TimetableData, border = 10, height = 600 }) {
    const tdRefsobj = {};
    ...
    for (let hoursSteps = dayStartHour; hoursSteps < dayEndHour;) {
        ...
        for (let dayCount = 0; dayCount <= maxDay;) {
            ...
            if (findedSeance) {
                ...
                timeTableTr.push(<TimetableTd key={key} seance={findedSeance} style={{ height: tdHeight, border: borderStyle, fontSize: '13px' }} onCreate={onTdCreate} rowspan={rowspan} hoursSteps={hoursSteps} columnNumber={dayCount} rowNumber={hoursStepsCounter} tdNumber={tdRefCount} />);
        ...
        timeTableBody.push(<div className='tr' style={{ height: rowHeight + 'px' }} key={hoursSteps}>
            ...
            {timeTableTr} 
                ...
                < div className="timeScaleElemntContainer h-100 position-absolute" key={hoursSteps} style={{left:`-${borderWidth/2}px`}     }>
                    <span className='timeScaleElemnt'>  {formatTime.format('HH:mm')} </span>
                    {currentSpecificTimeScale}
    ...
    return (
        ...
        <div>
            ...
            <div className='timetableHeader d-flex' >
                <table className='TimeTableTimeScaleSample'>
