...
function TimetableBreakTd({ currentBreak, style, onCreate, columnNumber, rowNumber }) {
    ...
    useEffect(function () {
        if (onCreate) {
            onCreate(tdRef.current, { columnNumber, rowNumber, currentBreak })

        }

    }, [])

    return <div className='td td_break' ref={tdRef} style={style} > {currentBreak.name} </div>
...
function TimetableTd({ seance, style, onCreate, hoursSteps, columnNumber, rowNumber, tdNumber, }) {
    ...
    if (hasRunOnce.current == false) {
        tdRefCount++;
        console.log('tdRefCount', tdRefCount)
...
function Timetable({ TimetableData, border = 10, height = 600, timeScaleWidth = '0%' }) {
    const tdRefsobj = {};
    ...
    function onBreakcreate(breakRef, props) {
        const { columnNumber, rowNumber, currentBreak } = props;
    ...
    let [reloadContent, setReloadContent] = useState(false);
    ...
    useEffect(() => {
        ...
        const onScreenResize = throttle(function onScreenResize() {
            ...
            console.log("screeennn resizeeeee");
    ...
    if (timeScaleWidth.includes('%')) {
        ...
        timeScaleWidth = timeScaleWidth.replace(/\s+/g, '');
        timeScaleWidthUnit = timeScaleWidth;
        // const timeScaleArr = timeScaleWidth.split(/(\d+)/);
        // timeScaleArr.shift()

        console.log("timescale spliiittttttt", timeScaleWidth);
        // const availableSpacepercentage = `calc(100% - ${timeScaleWidth})`


        // console.log(availableSpacepercentage)
    }
    let tdWidth;
    let tdWidthUnit
    let availableSpace
    if (timescaleWidthInPercentage) {
        ...
        tdWidthUnit = `calc((100% - ${timeScaleWidth}) / (${maxDay + 1}))`
        console.log('tdwidthhhh', tdWidthUnit)
    }
    ...
    for (let hoursSteps = dayStartHour; hoursSteps < dayEndHour;) {
                ...
                for (let matchingBreaksIndexe = 0; matchingBreaksIndexe < matchingBreaksIndexes.length;) {
                    ...
                    if (timeTablebreakAtIndex.only && timeTablebreakAtIndex.only.includes(dayCount)) {
                        ...
                        if (prevDayBreak.breakIndex == null || prevDayBreak.breakIndex !== matchingBreaksIndexe) {
                            ...
                            timeTableBreaksTr[timeTableBreaksCounter] = <TimetableBreakTd currentBreak={timeTablebreakAtIndex} style={{ width: tdWidth }} onCreate={onBreakcreate} rowNumber={hoursStepsCounter} />
            ...
            if (findedSeance) {
                ...
                function onTdCreate(tdRef, hoursSteps, columnNumber, rowNumber, tdNumber) {
                    ...
                    if (prevRowNumber != rowNumber) {
                        console.log('in prev row diff', tdRefsRow)
                        tdRefsRow
                        let oldTrRef = null;
                        ...
                        console.log(oldTrRef, prevHeightDiff)
                        if (oldTrRef !== null && prevHeightDiff != 0) {
                            ...
                            console.log('set modifiedTrsObj', modifiedTrsObj)
                        }
                    ...
                    console.log((rowNumber == lastRowNumber && tdNumber == tdRefCount && prevHeightDiff != 0), rowNumber, lastRowNumber, tdNumber, tdRefCount, prevHeightDiff)
                    if (rowNumber == lastRowNumber && tdNumber == tdRefCount && prevHeightDiff != 0) {
                        ...
                        console.log('last row modifiedTrsObj', modifiedTrsObj)

                    }


                }
                console.log('tdWidthUnit22', tdWidthUnit)
                ...
                if ((prevSeancesEnd[dayCount] == currentLine) && rowspan < 1) {
                    let leftTdOffset;
                    if (timescaleWidthInPercentage) {
                        leftTdOffset = tdWidth * dayCount + timeScaleWidth + "%"
                    } else {
                        leftTdOffset = `calc(((100% - ${timeScaleWidthUnit}) / 5) * ${dayCount} + ${timeScaleWidthUnit})`;
                        console.log('leftOffset___', leftTdOffset)
                    }
        ...
        if (timeTableBreaksTr.length > 0) {
            ...
            timeTableTr.push(
                <div className='breaks tr' style={{ height: "100%", width: availableSpace }} key={'breaks' + hoursSteps} >
        ...
        timeTableBody.push(<div className='tr' style={{ height: rowHeight + 'px' }} key={hoursSteps}>



            < div className=" timeScaleElemntContainer h-100 " key={hoursSteps} style={{ left: `-${borderWidth / 2}px`, minWidth: timeScaleWidthUnit, position: 'relative' }}>
                <span className='timeScaleElemnt'>  {formatTime.format('HH:mm')} </span>
                {currentSpecificTimeScale}
    ...
    return (
        ...
        <div>
            ...
            <div className='timetableHeader d-flex' >
                {reloadContent + 'test'}
                <div className='TimeTableTimeScale text-center' style={{ minWidth: timeScaleWidthUnit, maxWidth: timeScaleWidthUnit, overflowWrap: 'break-word' }}>
                    Hours
                </div>

                <div className='timeTableDayScale text-center d-flex' >
            ...
            <div className='timetableContainer  d-flex' style={{ minHeight: timetableHeight + 'px' }}>
                ...
                <div className="myTimetable">
                    ...
                    <div className="tbody" style={{ margin: `${borderWidth / 2}px` }}  >

                        <div className='outerBorder  position-absolute h-100 ' style={{
                        ...
                        <div className='innerBorder position-absolute h-100 ' style={{ border: borderStyle, width: availableSpace }}>
